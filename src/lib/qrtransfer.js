...
// browser, but it keeps this module runnable under jsdom, whose Blob has no
// stream() -- the same trap backupLink.js hit first.
...
const gzip = (bytes) => throughStream(bytes, new CompressionStream('gzip'));
const gunzip = (bytes) => throughStream(bytes, new DecompressionStream('gzip'));
...
export class FrameCollector {
  ...
  add(text) {
    // Keep in sync with MAGIC above -- the literal is baked into the regex.
